import type {
  ...
  ExecutorCompatibilitySignedManifestArtifactWriterPort,
...
import type {
  PublishExecutorCompatibilityReleaseManifestUseCaseInput,
  SignExecutorCompatibilityReleaseManifestBoundary,
} from "./contracts/index.js";

/** 按 Reader、Sign、Writer 顺序发布 Manifest Artifact。 */
export class PublishExecutorCompatibilityReleaseManifestUseCase {
  /** 创建 Manifest 发布用例。 */
  public constructor(
    /** 读取并重建 Draft 的端口。 */
    private readonly draftReader: ExecutorCompatibilityReleaseDraftReaderPort,
    /** 仅依赖 execute 方法的签名边界。 */
    private readonly signUseCase: SignExecutorCompatibilityReleaseManifestBoundary,
    /** create-only 写入 Artifact 的端口。 */
    private readonly writer: ExecutorCompatibilitySignedManifestArtifactWriterPort,
  ) {}

  /** 按 Reader、Sign、Writer 顺序执行并在失败时短路。 */
  public async execute(
    input: PublishExecutorCompatibilityReleaseManifestUseCaseInput,
  ): Promise<Result<ExecutorCompatibilityReleaseArtifactWriteResult, HarnessError>> {
    const draft = await this.draftReader.readManifestDraft({ draftFilePath: input.draftFilePath });
