...
| Workflow Core            | `partial`     | 固定 Cell、Failure 路由、Human 控制、Aggregate、Reducer、File Store、合法 Command、CodingTask Domain Core、CodingTask Store/Replay、Command Service、Worktree/Write Set 检查、Repository Lock、JournaledActionRunner、受控文件变更、Git Checkpoint、ImplementationSubmitted、版本化 Verification Command | Child 引用、恢复编排、Cell Runtime、Worktree 清理/重建、Verification 影响面 |
...
| Verification 与 Evidence | `partial`     | Evidence 基础类型、ProjectProfile、VerificationPlan、EvidenceBundle、fail-closed Mock、显式 Local Command Runner、版本化 Command/Journal 恢复、Git Revision 绑定、环境/超时/输出限制                                                                                                                     | 影响面、重试/Flaky、Waiver、Independent Verifier、多仓编排                  |
...
| 多仓写入与 Worktree      | `partial`     | 多仓身份、Profile、Write Set 规范化、Managed Worktree 创建/检查、Workspace/Repository 排他 Lock、单仓受控文件变更与 Git Checkpoint                                                                                                                                                                       | Workspace Registry、Worktree 清理/重建、跨仓 Saga、Compensation             |
| 生产安装、升级与卸载     | `designed`    | npm 发布物                                                                                                                                                                                                                                                                                               | Managed Files、Migration、Rollback                                          |

### 5.1 S3 CodingTask 进展修订

S3 当前已经完成 Domain Core、File Store/Replay、Command Gateway/Service、默认权威授权解析、Managed Worktree Provision/Inspector、Workspace/Repository 排他 Lock、Action 级执行锁和 Intent-first `JournaledActionRunner`、受控文件变更、可信 Repository Root、Git Checkpoint、ImplementationSubmitted Attempt 收口、Verification 版本化命令与 EvidenceBundle、fail-closed Mock 和显式 Local Command Runner。进入真实代码交付前仍必须补齐 Worktree 清理/重建、Verification 影响面选择、失败分类/重试和多仓编排，并在真实或可公开复现项目完成验证；在这些能力完成前，不得宣称 CodingTask 已能独立完成真实代码交付。
