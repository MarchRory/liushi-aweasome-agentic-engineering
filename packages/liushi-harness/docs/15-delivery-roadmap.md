...
| Executor Adapter         | `partial`     | CLI、Composition Root、Codex Probe、Hook Adapter、G0 InstallPlan dry-run/Apply、Installation Revision、Executor Compatibility Matrix Domain、真实 Codex CLI 0.144.5 Host Result v2、Host 7 条 Evidence Projector、固定 v2 Contract Suite 5 条 ContractTest、写入前 Projection Set 复验、双 Artifact Store、重算查询 CLI、确定性 Publication Bundle、create-only 原子输出 CLI、P3a G6 Domain、P3b Sigstore Sign/Offline Verify、P4a Manifest/Trust Profile 与 Manifest G6 Draft Domain | Attestation CLI、签名 Artifact Writer、Signed Manifest Artifact/Verify、Accepted Head、安装信任门、Claude-compatible/CatPaw、其他工具与 Executor |
| Hooks 与 Agent Runtime   | `partial`     | Gate、Canonical Event、Codex Projection/Wrapper、同 invocation 证据绑定和 Host Result v2 结果门                                                                                                                                                                                                                                                                                                                                                                                       | 其他平台、其他生命周期、Role Runtime                                                                                                             |
...
S3 当前已经完成 Domain Core、File Store/Replay、Command Gateway/Service、默认权威授权解析、Managed Worktree Provision/Inspector、未知 Provision Human 对账与下游 Guard、Workspace/Repository 排他 Lock、Action 级执行锁和 Intent-first `JournaledActionRunner`、受控文件变更、可信 Repository Root、Git Checkpoint、ImplementationSubmitted Attempt 收口、Checkpoint 后权威绑定 Verification Target Revision、Verification 版本化命令与 EvidenceBundle、G8 Profile-backed 单仓影响面、fail-closed Mock、显式 Local Command Runner、要求 Workspace/Repository/绝对 Root/Verification Mode 的生产 CLI 单仓 Cell、单仓 PRReadyArtifact 权威装配、Codex Managed File InstallPlan dry-run/G0 Apply/Installation Revision，以及 Executor Compatibility Matrix Domain、Host/Contract 双 Artifact Store 和重算查询 CLI。代码与测试已证明 CLI Binding、InstallPlan 和 Apply 在副作用前关闭式校验，并由生产 Bootstrap 映射到真实 Composition Root；真实 Git E2E 已验证 G1/G4 Human Approval、Checkpoint、Evidence Passed、PRReadyArtifact 确定性、跨实例零重复副作用和 dry-run Repository 零变更。固定公开项目 `unjs/defu@82632b66` 已通过真实 Tarball CLI、两个模拟 Gate、受管 Worktree、预编排单文件 Mutation、唯一 Checkpoint、两个 Required Checks、Passed Evidence、PRReadyArtifact 和跨进程幂等正路径验证。

Codex 兼容性路径现已把 Host Projector 的 7 条 Evidence 与确定性 Contract Suite 的 5 条 ContractTest 合并为 12 条；真实 Codex CLI `0.144.5`、Windows x64、Interactive TUI 的受验输入已全部 Passed，编译为本机 `compatible` Matrix，并通过 Query 重算。确定性 Publication Bundle 已绑定 Tarball、源码 Revision、双 Artifact、12 条 Evidence、Policy 和 Matrix，并可由 create-only CLI 原子写出；P3a 已新增固定发布者身份、Release Candidate、真实 Human G6 摘要复验和未签名 in-toto Draft，P3b 已补齐官方 Sigstore DSSE 签名、内容寻址 Artifact 与显式 Trusted Root 纯离线验证，P4a 已交付确定性 Manifest 与 Consumer Trust Profile Domain，P4b1 已收敛通用 in-toto Sigstore 边界，P4b2 已交付独立 Manifest Human G6 Draft 与 in-toto Statement Domain。没有 Signed Manifest Artifact、显式 Trust Profile 离线验签、安装选择门、ProductionE2E、`modelId` 或 `permissionMode` 时绝不声明 Production。证据不把自动化 Approval 计作日常业务 Human 决策或 HTT，也不证明 Agent 自主编码和失败修复循环。下一步是 Signed Manifest Artifact 与离线验证链，再补 Attestation/Manifest CLI/Writer 和安装选择门；之后进入 Claude-compatible/CatPaw Adapter。Rollback/Uninstall、Worktree 清理/重建、失败分类/受限重试和多仓编排仍是后续能力。
