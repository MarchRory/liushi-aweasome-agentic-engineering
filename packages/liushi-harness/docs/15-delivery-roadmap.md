...
| Workflow Core            | `partial`     | 固定 Cell、Failure 路由、Human 控制、Aggregate、Reducer、File Store、合法 Command、CodingTask Domain Core、CodingTask Store/Replay、Command Service、Worktree/Write Set 只读检查、VerificationPlan/Port/EvidenceBundle/Mock | Child 引用、恢复编排、Cell Runtime、Worktree 生命周期、真实 Verification Runner     |
...
| Verification 与 Evidence | `partial`     | Evidence 基础类型、ProjectProfile、VerificationPlan 校验、Verification Port、EvidenceBundle 装配、fail-closed Mock Executor                                                                                                 | 真实 Command Runner、影响面选择、重试/Flaky、Waiver、Independent Verifier、多仓编排 |
