...
Codex 兼容性路径现已把 Host Projector 的 7 条 Evidence 与确定性 Contract Suite 的 5 条 ContractTest 合并为 12 条；真实 Codex CLI `0.144.5`、Windows x64、Interactive TUI 的受验输入已全部 Passed，编译为本机 `compatible` Matrix，并通过 Query 重算。确定性 Publication Bundle 已绑定 Tarball、源码 Revision、双 Artifact、12 条 Evidence、Policy 和 Matrix，并可由 create-only CLI 原子写出；P3a 已新增固定发布者身份、Release Candidate、真实 Human G6 摘要复验和未签名 in-toto Draft，P3b 已补齐官方 Sigstore DSSE 签名、内容寻址 Artifact 与显式 Trusted Root 纯离线验证，P4a 已交付确定性 Manifest 与 Consumer Trust Profile Domain。没有 Manifest G6/DSSE、安装选择门、ProductionE2E、`modelId` 或 `permissionMode` 时绝不声明 Production。证据不把自动化 Approval 计作日常业务 Human 决策或 HTT，也不证明 Agent 自主编码和失败修复循环。下一步是 Manifest G6/DSSE 与离线选择链，再补 Attestation/Manifest CLI/Writer 和安装选择门；之后进入 Claude-compatible/CatPaw Adapter。Rollback/Uninstall、Worktree 清理/重建、失败分类/受限重试和多仓编排仍是后续能力。
