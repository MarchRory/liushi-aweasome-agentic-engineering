...
| Verification 与 Evidence | `partial`     | Evidence 基础类型、ProjectProfile、VerificationPlan、EvidenceBundle、fail-closed Mock、显式 Local Command Runner、Git Revision 绑定、环境/超时/输出限制                                                                                                             | 版本化 Command/恢复、影响面、重试/Flaky、Waiver、Independent Verifier、多仓编排 |
...
S3 当前已经完成 Domain Core、File Store/Replay、Command Gateway/Service、默认权威授权解析、Managed Worktree Provision/Inspector、Workspace/Repository 排他 Lock、Action 级执行锁和 Intent-first `JournaledActionRunner`、VerificationPlan/EvidenceBundle、fail-closed Mock 和显式 Local Command Runner。进入真实代码交付前仍必须补齐 Worktree 清理/重建、受控代码写入、Verification 版本化命令/恢复、影响面选择、失败分类/重试和多仓编排；在这些能力完成前，不得宣称 CodingTask 已能独立完成真实代码交付。
