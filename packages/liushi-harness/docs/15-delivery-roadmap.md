...
| Workflow Core            | `partial`     | 固定 Cell、Failure 路由、Human 控制、Aggregate、Reducer、File Store、合法 Command、CodingTask Domain Core、CodingTask Store/Replay、Command Service、Worktree/Write Set 只读检查 | Child 引用、恢复编排、Cell Runtime、Worktree 生命周期、Verification Runner   |
...
S3 当前已经完成 Domain Core、File Store/Replay、Command Gateway/Service、默认权威授权解析和 Worktree/Write Set 只读检查 Port。进入真实代码交付前仍必须补齐 Worktree 生命周期与 Repository Lock、Verification Port、EvidenceBundle 和 Mock Executor；在这些能力完成前，不得宣称 CodingTask 已能独立完成真实代码交付。
