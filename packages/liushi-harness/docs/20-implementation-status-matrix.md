...
- Closeout Recovery 已提供全链只读 Assessment 和独立 Process State：从 v3 State 重建并重新解析原 Closeout Command，复验 Activation、CodingTask、Session Binding、可信 Root 与 Managed Worktree，按 `Absent`/`Present`/`Unknown` 和 fresh Snapshot 现场只允许唯一 `RetryOnce` 或 `BindExisting`，生成不包含自身与 Evidence ID 的 canonical Assessment Digest，并以严格六状态绑定 Human Command 身份、原 Closeout、Snapshot、ChangeSet 与 Checkpoint successor。File Store、Human Command parser、锁内重评估和执行仍未实现。
