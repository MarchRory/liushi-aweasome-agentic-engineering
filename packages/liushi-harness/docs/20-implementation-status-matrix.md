...
| 09 Hooks 与 Agent Runtime   | 已完成   | `partial`     | Canonical Pre/PostAction、Codex `apply_patch` Projection/Wrapper、PlanRisk/Human Gate 授权、Dispatcher、Action Journal、JournaledActionRunner 与 Trace                                                                                                                                                                          | Worktree/Verification Runner 接入、其他平台 Projection、自动安装、其他生命周期、Role Runtime 和 Human Battle                 |
...
因此，表格中 CodingTask Store/Command 的旧描述以本节为准；Worktree 创建/管理、受控写入、Worktree/Verification 对 Journaled Runner 的接入、真实 Verification Command Runner、影响面选择、重试/Flaky、Waiver、多仓编排和 Studio 仍未实现。
