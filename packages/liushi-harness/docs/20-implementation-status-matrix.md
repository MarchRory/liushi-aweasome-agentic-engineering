...
- Closeout Recovery 已提供全链只读 Assessment、独立 Process State、File Store、Human Command Contract 与应用层 Handler/Service：从 v3 State 重建并重新解析原 Closeout Command，复验 Activation、CodingTask、Session Binding、可信 Root 与 Managed Worktree，按 `Absent`/`Present`/`Unknown` 和 fresh Snapshot 现场只允许唯一 `RetryOnce` 或 `BindExisting`，生成不包含自身与 Evidence ID 的 canonical Assessment Digest，以严格六状态绑定 Human Command 身份、原 Closeout、Snapshot、ChangeSet 与 Checkpoint successor，通过 create-only、独立短时锁、精确 CAS、严格重建和未知结果分类持久化恢复进度，并以 Human-only Envelope、精确四字段 Payload 与 canonical request digest 重算冻结授权输入。Handler 在 Repository Lock 内 fresh reassess，仅允许本次 `Approved -> Executing` CAS winner 调用一次 Checkpoint execute；`Executing` 重放和 `BindExisting` 永不 execute，Checkpoint 必须再次匹配锁定的 Snapshot/ChangeSet，全部写入或锁释放未知结果关闭式失败。Effective Resolver、Composition Root/CLI 与真实 Git 恢复 E2E 尚未实现；共享路径祖先替换 TOCTOU 也必须在不可信目录真实 Pilot 前独立关闭。
