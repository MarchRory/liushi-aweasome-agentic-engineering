...
| 08 Executor Adapter         | 已完成   | `partial`     | Codex Probe、Hook Binding、Pre/Post Adapter、CLI Wrapper、`hooks.json` 投影、G0 InstallPlan dry-run、Fixture 测试、固定公开项目生产 CLI Cell Smoke，以及 Windows/Codex `0.144.0-alpha.4` 交互式 TUI 的同会话 Host 正负路径                                                                                                                                            | G0 Apply/Revision、Codex 版本/平台矩阵、Claude/CatPaw Adapter、Role Invocation                                                  |
...
| 14 生产 SOP                 | 已完成   | `partial`     | npm、Tarball 干净安装 Smoke、ESM/CJS、双 CLI Bin、Doctor、Task、Artifact、Approval、Rule、Scanner、Profile Compile、Codex Probe、G0 InstallPlan dry-run、固定公开项目 Cell 证据，以及交互式 Host Smoke Prepare/Result Gate                                                                                                                                            | G0 Apply、企业自动安装、自动 Task Delivery、Executor、Workflow、Memory、Learning 和长期治理命令                                 |
...
因此，表格中 CodingTask Store/Command 的旧描述以本节为准；固定公开项目的预编排 Mutation CLI 正路径和精确 Codex TUI Host Scope 已形成动态验证快照，但 Worktree 清理/重建、Import Graph、重试/Flaky、Waiver、多仓影响传播、真实 Agent 编码、企业安装和 Studio 仍未实现。
...
- Codex Managed File dry-run：平台无关 InstallPlan Domain、隔离 Runtime Store 持久化、Existing Human File 保护、Manifest Claim Provenance、Windows 路径身份和零 Repository 写入 E2E。
...
- G0 Apply/Installation Revision/Rollback/Uninstall、Codex Host Hook 自动安装、其他工具/版本/平台验证、Claude-compatible 或 CatPaw Adapter。
