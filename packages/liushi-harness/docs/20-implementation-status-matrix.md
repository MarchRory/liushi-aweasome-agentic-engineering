...
| 14 生产 SOP                 | 已完成   | `partial`     | npm、Tarball 干净安装 Smoke、ESM/CJS、双 CLI Bin、Doctor、Task、Artifact、Approval、Rule、Scanner、Profile Compile、Codex Probe、PRD 只读 `requirement analyze`、G0 InstallPlan dry-run、G0 Apply、Installation Revision、固定公开项目 Cell 证据，以及交互式 Host Smoke Prepare/Result Gate                                                                                                                                                                                                                                                                                                                                                                                   | Rollback/Uninstall、企业自动安装、Human Battle 持久化、自动 Task Delivery、完整 Workflow、Memory、Learning 和长期治理命令                                                                                                   |
| 15 交付路线                 | 已完成   | `implemented` | 能力门、决策门、完成门、真实项目验证口径和 ADR-015 价值优先黄金路径                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | 后续切片按 Human Battle、PlanRisk、CodingTask 接线顺序推进                                                                                                                                                                  |
...
| 21 Workflow 产品与技术方案  | 已完成   | `partial`     | PRD 只读 `requirement analyze`、严格 Codex 线格式、Requirement Proposal 领域复验、Human Battle 问题投影、真实 Codex 零 Repository 写入烟测，以及 Command Gateway、Revision/Context、Replay、Timeline、Action Journal、Trace、Canonical Hook Core、RequirementWorkflow/CodingTask Aggregate 与 File Store、权威授权、受信预编排 CodingTask CLI Cell、Managed Worktree、受控文件变更、Git Checkpoint、Verification、PRReadyArtifact、Session Closeout/Recovery/Completion、真实 Git E2E 与公开项目 Pilot                                                                                                                                                                        | Human Battle 修订/确认与 Artifact 接线、PlanRisk 生成、真实企业 Pilot 与对照量化、多仓交付、Child Workflow、完整 RequirementWorkflow Cell Runtime、其他平台 Hook/Executor、实时 Trace/Exporter、Worktree 清理/重建与 Studio |
...
- Application 强制单仓边界、空 `humanAnswers` 和 Claim 到 Evidence 的引用完整性；`unknowns` 直接投影为 Human Battle 问题。
- 真实 Codex CLI `0.145.0` 与 `gpt-5.6-sol` 烟测在 74.6 秒内成功返回 `human_battle_required`，目标 Repository 前后均为干净状态。
- 当前切片只生成 Report-only Proposal。Human 修订、确认、Task/Artifact 持久化和后续 PlanRisk 接线仍未实现，也没有企业项目自动化比例结论。
