...
| 07 Workspace 与多仓         | 已完成   | `partial`     | 多仓身份、只读扫描、依赖歧义、Profile Proposal、G8、Profile Bundle、Managed Worktree 创建/检查、未知 Provision Human 对账、Workspace/Repository 排他 Lock、单仓受控文件变更、可信 Repository Root 与 Git Checkpoint                                                                                            | WorkspaceGraph Registry、未闭合 Worktree Journal 下游写阻断、Worktree 清理/重建和跨仓 Saga                                |
...
| 21 Workflow 产品与技术方案  | 已完成   | `partial`     | Command Gateway、Revision/Context、Replay、Timeline、Action Journal、Trace、Canonical Hook Core、RequirementWorkflow/CodingTask Aggregate 与 File Store、权威授权、Managed Worktree、未知 Provision Human 对账、受控文件变更、Git Checkpoint、ImplementationSubmitted、版本化 Verification Command、单仓影响面 | CLI 迁移、Child Workflow、平台 Hook/Executor、实时 Trace/Exporter、未闭合 Journal 下游写阻断、Worktree 清理/重建与 Studio |
...
因此，表格中 CodingTask Store/Command 的旧描述以本节为准；未闭合 Worktree Journal 的下游写阻断、Worktree 清理/重建、Import Graph、重试/Flaky、Waiver、多仓影响传播、真实项目生产验证和 Studio 仍未实现。
...
- 现有 CLI 写命令向 Command Gateway 的完整迁移、Child Workflow、Cell Runtime、未闭合 Worktree Journal 下游写阻断、Worktree 清理/重建和多仓 Verification 影响传播。
...
Workflow 产品与技术方案已经对齐，持久化 Command Gateway、Revision/Context、Golden Replay、Timeline、Action Journal、JournaledActionRunner、Trace、Canonical Hook Core、Workflow Kernel 和 CodingTask Domain Core 已通过测试。Codex `apply_patch` Adapter、Binding、配置投影、CLI Wrapper 和静态 Probe 已完成 Fixture 验证；CodingTask Store/Command、Managed Worktree Provision/Inspector、未知 Provision Human 对账、Repository Lock、受控文件变更、可信 Repository Root、Git Checkpoint、ImplementationSubmitted、版本化 Verification Command/EvidenceBundle、G8 Profile-backed 单仓影响面、fail-closed Mock 和显式 Local Command Runner 已完成。下一步进入未闭合 Worktree Journal 的全局下游写阻断、Human-gated Worktree 安全清理与显式重建，并等待 Human 在受信任项目执行 Hook Smoke/Negative Test。Claude-compatible/CatPaw、自动安装和 Role Invocation 仍未实现。
