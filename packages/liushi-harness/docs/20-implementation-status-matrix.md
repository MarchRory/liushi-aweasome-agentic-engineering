...
| 08 Executor Adapter         | 已完成   | `partial`     | Codex Probe、Hook Binding、Pre/Post Adapter、CLI Wrapper、`hooks.json` 投影、G0 InstallPlan dry-run、G0 Apply、Installation Revision、Executor Compatibility Matrix Domain、真实 Codex CLI 0.144.5 Host Result v2、Host 7 条 Evidence Projector、固定 v2 Contract Suite 5 条 ContractTest、双 Artifact Store、重算查询 CLI、确定性 Publication Bundle、create-only 原子输出 CLI、P3a G6 Domain、P3b/P4b3 包内 Sigstore Sign、P4a-P4b3 Manifest/Trust Profile、Manifest G6、可信 Release Approval Authority、Signed Manifest、公开离线 Verify、P4c1 严格 Artifact Reader/create-only Writer、P4c2a 企业 HTTPS Authority Adapter、Fixture 测试和固定公开项目生产 CLI Cell Smoke | P4c2b 隔离 Release Host 与 CLI 接线、Accepted Head、安装信任门、Rollback/Uninstall、Claude/CatPaw Adapter、Role Invocation                                                                   |
...
| 12 Verification 与 Evidence | 已完成   | `partial`     | G8 Profile Check、单仓确定性影响面、带 Profile/Approval/Rule 来源的 VerificationPlan v2、Local Command Runner、版本化 Verification Command/Journal 恢复、Git Revision 绑定、环境/超时/输出限制、EvidenceBundle 强一致 Store、Plan Digest 复验、Session Completion 到 PR-ready 串联、幂等/冲突/篡改检测                                                                                                                                                                                                                                                                                                                                                                        | Import Graph、Flaky、G5、Independent Verifier 和多仓验证                                                                                                                                     |
| 13 Learning 与 Knowledge    | 已完成   | `designed`    | 无产品运行时能力                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | Candidate Store、Eval、Promotion、Retrieval、Curator 和 Skill 改进                                                                                                                           |
| 14 生产 SOP                 | 已完成   | `partial`     | npm、Tarball 干净安装 Smoke、ESM/CJS、双 CLI Bin、Doctor、Task、Artifact、Approval、Rule、Scanner、Profile Compile、Codex Probe、PRD 只读 `requirement analyze`、无摘要输入的 `requirement confirm`、G0 InstallPlan dry-run、G0 Apply、Installation Revision、固定公开项目 Cell 证据，以及交互式 Host Smoke Prepare/Result Gate                                                                                                                                                                                                                                                                                                                                               | Rollback/Uninstall、企业自动安装、多轮 Human Battle 状态、自动 Task Delivery、完整 Workflow、Memory、Learning 和长期治理命令                                                                 |
| 15 交付路线                 | 已完成   | `implemented` | 能力门、决策门、完成门、真实项目验证口径和 ADR-015 价值优先黄金路径                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | 后续切片按 PlanRisk、CodingTask、Learning 接线顺序推进                                                                                                                                       |
...
| 21 Workflow 产品与技术方案  | 已完成   | `partial`     | PRD 只读 `requirement analyze`、Requirement/G1 Human 确认、已批准 Requirement 驱动的只读 `plan-risk analyze`、Business Logic/G2 与 PlanRisk/G4 分阶段 Human Review、历史逻辑 Artifact Digest 内部绑定、R4 拒绝、Core 禁止 Agent 直接提交 Planning Artifact、严格 Codex 线格式、真实文件 Store E2E，以及 Command Gateway、Revision/Context、Replay、Timeline、Action Journal、Trace、Canonical Hook Core、RequirementWorkflow/CodingTask Aggregate 与 File Store、权威授权、受信预编排 CodingTask CLI Cell、Managed Worktree、受控文件变更、Git Checkpoint、Verification、PRReadyArtifact、Session Closeout/Recovery/Completion、真实 Git E2E 与公开项目 Pilot                 | 真实企业 Pilot 与对照量化、多轮 Human Battle 状态、多仓交付、Child Workflow、完整 RequirementWorkflow Cell Runtime、其他平台 Hook/Executor、实时 Trace/Exporter、Worktree 清理/重建与 Studio |

### 3.1 PRD 分析与 Human 确认切片

- `requirement analyze` 接收绝对 PRD 文件、Workspace、Repository、绝对 Repository Root、显式模型和可选 Codex executable。
- Production Composition Root 通过 `RequirementAnalysisAgent` Port 装配 Codex Adapter；Application 只依赖 executor-neutral Contract。
- Codex 运行固定使用 ephemeral、read-only Sandbox 和禁用 Hooks，不创建 Task、不写 Runtime Store、不修改 Repository。
- Codex 专用线格式将领域可选 Evidence 字段表示为 `null`，满足 Strict Structured Outputs 的全字段 required 约束；Adapter 归一化后仍由现有 `RequirementContractProposal` Schema 复验。
- Application 强制单仓边界、空 `humanAnswers`、最多五个非重复问题和 Claim 到 Evidence 的引用完整性；`unknowns` 直接投影为 Human Battle 问题与同文件 `reviewDraft`。
- 真实 Codex CLI `0.145.0` 与 `gpt-5.6-sol` 烟测在 74.6 秒内成功返回 `human_battle_required`，目标 Repository 前后均为干净状态。
- Human 填写 Review Draft 并运行 `requirement confirm` 后，Application 在写入前校验 Actor、问题覆盖、修订 Proposal 与 Repository，再以内部幂等身份复用现有 Artifact 和 G1 Approval；CLI 不接收或输出摘要。
- 当前切片需要已有 Task，不自动生成 PlanRisk 或启动编码；多轮 Human Battle 状态和企业项目自动化比例仍未验证。

### 3.2 PlanRisk 与历史业务逻辑确认切片

- `plan-risk analyze` 只读加载已通过 G1 的当前 Requirement，并通过 Workspace/Repository 身份从启动期可信映射解析 Repository Root；Application 不接受调用方直接注入任意路径。
- Codex Adapter 固定使用 ephemeral、禁用 Hooks、read-only Sandbox、严格 Structured Output、十分钟超时和输出上限；Business Logic 阻断问题在 Schema 与 Prompt 中同时限制为最多五个，Application 再次复验。
- 历史逻辑变更必须先生成 Business Logic Review，由 Human 完整回答原始问题并确认 G2；再次分析后才允许生成绑定该 Artifact Digest 的 PlanRisk。Digest 由 Task Replay 在内部注入，不暴露为 Human 输入。
- R1 仍需 Human 确认后才提交；R2/R3 在同一次确认中完成 G4；R4 在任何写入前关闭式拒绝。Core 禁止 Agent Actor 直接提交 Business Logic/PlanRisk；Human 手工 `artifact propose` 保留为高级显式确认路径并继续执行既有 Gate Policy。
- 真实文件 Store E2E 覆盖 R1、历史逻辑 G2/R3/G4、顺序幂等重放、公共入口防绕过、输出无 Digest 和 Repository 零修改。真实 `gpt-5.6-sol` 烟测证明模型调用、Structured Output 和领域复验可用，但当前桌面内嵌 Windows Sandbox 无法启动仓库读取命令，因此该结果只按受限通过记录，不能替代企业项目 Pilot。

### 3.3 本轮 CodingTask 与 Session S1-S3 修订

本轮代码已将 `CodingTask Domain Core` 扩展为可调用的编码 Cell 持久化切片，并完成 Session S1 Activation 与 S2 Action Admission：
...
- `completeCodingTaskSessionDelivery` 已在 Delivery 后重读权威 CodingTask，重新编译精确 G8 Profile、解析当前 Task Rule、选择带五项来源引用的 Plan，通过平台路径端口复验受管 Worktree，再复用版本化 Verification、Evidence Store 与 PR-ready 装配尾链。通过和失败路径均已证明跨 Application 精确重放。
...
- `cell run` 已要求 Workspace、Repository、绝对 Repository Root 和 Verification Mode；生产 Bootstrap 将绑定映射到静态 Root Resolver 与 Composition Root，并在任何副作用前校验 Manifest Create 身份、全部 Repository Runtime Root 和受管 Verification Worktree Root。该入口保留为受信宿主预编译 Plan 的兼容路径，不面向不受信 Agent；Session golden path 使用 `completeCodingTaskSessionDelivery`。
- Session Activation S1 已提供独立 Domain、不可变 Activation Record/File Repository、Session 级跨进程 Lease、`Create -> Provision -> StartAttempt` 后的权威读取、耐久化 `outcome_unknown`、CLI `coding-task session activate`、跨实例幂等复用和真实 Git E2E。
- Session Action Admission S2 已提供 Hook Binding v2、Admission State/File Store、非等待 Lease、Activation 自动初始化，以及同一 Lease 内的 PreAction 权威身份/PlanRisk/G2/G4/Write Set 复验、pending、健康 v2 Intent 和 `admittedActionIds` 提交。PostAction 会复验 v2 Intent，记录 Trace、绑定 Trace 摘要的 v2 Observation 与受其因果绑定的 Resolution；重放保持幂等。缺失 Post、WaitingHuman、RetryPermitted、pending 和跨 Store 提交未知会阻断 Closing 或把 `waiting_agent`/`closing` 迁移到 `outcome_unknown`。
- S3 已提供提交前权威 Snapshot、提交后 Commit ChangeSet 独立重建、ChangeSet/Checkpoint 联合摘要、提交前漂移零副作用拒绝和已存在 Checkpoint 幂等恢复。Closeout Process State/File Store 已按精确版本持久化完整 Snapshot、Action Evidence 与双向绑定 Checkpoint，并提供 create-only、短时锁、CAS、严格重建和未知结果分类。`CodingTaskSessionCloseoutManager` 已接入 `HarnessApplication`，在 Repository Lock 内 fresh 读取 Activation、CodingTask、Binding、Repository Root 和 Managed Worktree，校验 Command Actor、时间、Attempt、Gate 与身份，幂等推进 Closeout State，完成 `beginClosing`、Action Coverage、权威 Snapshot、`persistSnapshot`、Checkpoint execute + inspect 与 `bindCheckpoint`，严格停止在 `CheckpointBound`。
- Closeout Recovery 已提供全链只读 Assessment、独立 Process State、File Store、Human Command Contract、应用层 Handler/Service、Effective Resolver、生产 Composition Root 与 CLI：从 v3 State 重建并重新解析原 Closeout Command，复验 Activation、CodingTask、Session Binding、可信 Root 与 Managed Worktree，按 `Absent`/`Present`/`Unknown` 和 fresh Snapshot 现场只允许唯一 `RetryOnce` 或 `BindExisting`，生成不包含自身与 Evidence ID 的 canonical Assessment Digest，以严格六状态绑定 Human Command 身份、原 Closeout、Snapshot、ChangeSet 与 Checkpoint successor，通过 create-only、独立短时锁、精确 CAS、严格重建和未知结果分类持久化恢复进度，并以 Human-only Envelope、精确四字段 Payload 与 canonical request digest 重算冻结授权输入。Handler 在 Repository Lock 内 fresh reassess，仅允许本次 `Approved -> Executing` CAS winner 调用一次 Checkpoint execute；`Executing` 重放和 `BindExisting` 永不 execute，Checkpoint 必须再次匹配锁定的 Snapshot/ChangeSet，全部写入或锁释放未知结果关闭式失败。Resolver 对原 `CheckpointBound` 执行零 Recovery 读取短路，并只在 Recovery 与原终态的身份、version、完整 State Digest、Snapshot、ChangeSet 和路径绑定全部一致时返回恢复 Checkpoint。CLI 只暴露 `assess`、完整 Human Command `recover` 与只读 `effective`，不允许直接指定 Resolution 或写 State。确定性真实 Git E2E 已覆盖 `NotApplied -> RetryOnce` 的零到唯一 Commit，以及真实 Commit 后观察未知的 `BindExisting` 零新增 Commit，并验证精确 Human Command、稳定重放回执、Effective Checkpoint、干净 Worktree 和原 Closeout 字节不变；共享路径祖先替换 TOCTOU 仍必须在不可信目录真实 Pilot 前独立关闭。
- `waiting_agent` 只表示可以继续竞争 Session Action Admission，不是脱离 Hook 的文件写入授权。CLI Root/Actor 仍只是操作员声明；真实 Pilot 前必须由企业包装器或 Human 批准 Registry 注入可信 Runtime Binding。Windows 目录 `fsync` 只在 Infrastructure 平台层把精确 `win32 + EPERM + fsync` 识别为等价耐久，其他 best-effort 状态仍为降级。

因此，表格中 CodingTask Store/Command 的旧描述以本节为准；固定公开项目的预编排 Mutation CLI 正路径已有动态验证快照。历史 Codex Host Smoke 只保留旧版结果，不能作为 Host Result v2 Artifact；2026-07-16 已重新执行真实 Codex CLI `0.144.5` v2 Host，并通过 Compile/Query 形成本机 `compatible` Matrix。确定性 Publication Bundle、create-only 原子输出 CLI、P3a G6 Domain、P3b/P4b3 包内 Sign，以及 P4a-P4b3 Manifest/Trust Profile、Manifest G6、可信 Release Approval Authority、Signed Manifest 与公开离线 Verify 已实现。P4c1 已实现包内严格 Artifact Reader/create-only Writer，按 canonical 字节、精确长度、领域摘要和受信输出根关闭式校验；P4c2a 已实现企业 HTTPS Authority Adapter。公共 npm API 与 `HarnessApplication` 继续关闭签名面；Release Host、Accepted Head 和安装信任门已冻结，不再作为下一实现门。Closeout、Recovery、Delivery Submission 与权威 Verification/Evidence/PRReady Completion 已完成真实 Git E2E；真实 Codex App Server 进一步在固定公开项目、单文件 Write Set 和唯一 Checkpoint 下通过 Pilot。Completion CLI 负责把同一尾链暴露给受信宿主，Pilot Completion/Settlement 薄编排已通过确定性测试推进到 `completed`；企业项目 Pilot、量化、Worktree 清理/重建、Import Graph、重试/Flaky、Waiver、多仓影响传播和 Studio 仍未实现。

### 3.3 Managed File G0 Apply / Installation Revision
...
### 3.4 Executor Compatibility Matrix 与 Codex 证据投影
...
- CodingTaskSessionCloseoutManager：在 Repository Lock 内 fresh 读取并校验 Closeout 权威上下文，幂等创建/加载 State，执行 Action Coverage、Snapshot 和 ChangeSet-bound Checkpoint，严格停止在 `CheckpointBound`；前副作用只读 `IoFailure`/`LockUnavailable` 可安全重试，确定性前置失败为 `Blocked`，Checkpoint、State commit 或 Lock release 结果不可证明时为 `OutcomeUnknown` 或稳定不可重试错误。
- CodingTaskSessionCloseoutRecoveryAssessment：只读复验原 Closeout Command、业务身份、Checkpoint 三态与必要的 fresh Snapshot，向 Human 投影唯一 Resolution、稳定诊断和 canonical Assessment Digest；不创建恢复记录、不获取 Repository Lock、不执行 Checkpoint。
- CodingTaskSessionCloseoutRecoveryCommandHandler/Service：经 Application Command Gateway 接受严格 Human Command，在 Repository Lock 内重评估、create-only/CAS 推进独立 Recovery State，仅允许 CAS winner 至多一次执行 Checkpoint，并将 Executing 重放限制为只读 inspect。
...
- 自动 PlanRisk、Implementation、完整 Verification 和 Learning Workflow。
...
- 完整 Codex Agent Host：当前已完成 Runtime Isolation、Session Flags、App Server Runner 与零模型 Preflight；Application Host 生命周期仍未实现。
...
Workflow 产品与技术方案已经对齐，持久化 Command Gateway、Revision/Context、Golden Replay、Timeline、Action Journal、JournaledActionRunner、Trace、Canonical Hook Core、Workflow Kernel 和 CodingTask Domain Core 已通过测试。Codex `apply_patch` Adapter、Binding、配置投影、CLI Wrapper、静态 Probe、G0 InstallPlan dry-run、G0 Apply、Installation Revision、Host Result v2 验证、Host 7 条 Evidence Projector、固定 v2 Contract Suite 5 条 ContractTest、写入前 Projection Set 复验、双 Artifact 内容寻址 Store 和重算查询 CLI 已完成实现；CodingTask Store/Command、显式可信单仓绑定的生产 CodingTask CLI Cell、Managed Worktree Provision/Inspector、未知 Provision Human 对账与下游 Guard、Repository Lock、受控文件变更、可信 Repository Root、Git Checkpoint、ImplementationSubmitted、Checkpoint 后权威绑定 Verification Target Revision、版本化 Verification Command/EvidenceBundle、G8 Profile-backed 单仓影响面、fail-closed Mock、显式 Local Command Runner、单仓 PRReadyArtifact 和 CodingTaskSessionCloseoutManager 已完成。既有 CodingTask Cell 的真实 Git E2E 已通过生产 Bootstrap Local Command 映射、默认 Human Gate、PR-ready 装配与 dry-run 零 Repository 写入；固定公开项目 Smoke 已通过独立 Tarball Consumer 驱动 `unjs/defu@82632b66` 完成预编排单文件 Mutation 的 CLI 正路径、Passed Evidence、PRReadyArtifact、唯一 Commit 与跨进程幂等。

真实 Codex CLI `0.144.5`、Windows x64、Interactive TUI 的 12 条 Evidence 已全部 Passed，编译为本机 Compatible Matrix，并通过 Query 重算；Contract Failed Evidence 仍形成 Unsupported。确定性 Publication Bundle 已绑定双 Artifact、Evidence、Policy、Matrix、Tarball 与源码 Revision，并可由 create-only CLI 原子写出；P3a 已能生成绑定精确发布者身份、目标和 Human G6 Approval 的 in-toto Draft，P3b/P4b3 的包内签名链已能在可信 Authority 返回权威记录后生成真实 Sigstore DSSE Bundle 与 Signed Artifact，并以显式 Trusted Root 完成禁网离线验证；P4a-P4b3 已交付确定性 Manifest、Consumer Trust Profile、独立 Manifest Human G6、可信 Release Approval Authority、Signed Manifest Artifact 和完整双 DSSE 离线验证链。P4c1 已交付包内严格 Artifact Reader 和受信输出根约束下的 create-only Writer；它们拒绝重复 JSON Key、非法 UTF-8、非 canonical 字节、symlink/junction、精确长度或领域摘要漂移，且没有任意企业文件大小预算。P4c2a 已交付固定 HTTPS endpoint、Authority 标识、Bearer 凭据和超时的企业 Authority Adapter。当前 npm 根和 `HarnessApplication` 不暴露 Signer、Sign UseCase 或 Authority 工厂；Release Host、Accepted Head 和安装信任门已冻结，不再作为下一实现门。Closeout Recovery、Delivery Submission、权威 Verification/Evidence/PRReady Completion 和真实 Codex 公开项目 Pilot 已完成；`coding-task session complete` 复用同一权威链。仓库内 `Pilot Case v1` 已提供首个企业需求的本地配置入口，并以无摘要 Human Facts 完成到 `completed` 的薄编排，但不属于 npm 发布物，也不构成企业运行证据。下一步运行真实脱敏 Case 并量化，不切入 Release Host、Studio 或其他 Executor。Rollback/Uninstall、Worktree 清理/重建、失败分类/受限重试和多仓编排仍属后续能力。
