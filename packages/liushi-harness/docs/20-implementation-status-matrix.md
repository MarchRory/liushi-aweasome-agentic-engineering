...
- Closeout Recovery 已提供全链只读 Assessment：从 v3 State 重建并重新解析原 Closeout Command，复验 Activation、CodingTask、Session Binding、可信 Root 与 Managed Worktree，按 `Absent`/`Present`/`Unknown` 和 fresh Snapshot 现场只允许唯一 `RetryOnce` 或 `BindExisting`，并生成不包含自身与 Evidence ID 的 canonical Assessment Digest。Recovery Process State/File Store、Human Command、锁内重评估和执行仍未实现。
