...
- Closeout Recovery 已提供全链只读 Assessment、独立 Process State 与 File Store：从 v3 State 重建并重新解析原 Closeout Command，复验 Activation、CodingTask、Session Binding、可信 Root 与 Managed Worktree，按 `Absent`/`Present`/`Unknown` 和 fresh Snapshot 现场只允许唯一 `RetryOnce` 或 `BindExisting`，生成不包含自身与 Evidence ID 的 canonical Assessment Digest，以严格六状态绑定 Human Command 身份、原 Closeout、Snapshot、ChangeSet 与 Checkpoint successor，并通过 create-only、独立短时锁、精确 CAS、严格重建和未知结果分类持久化恢复进度。Human Command parser、锁内重评估和执行仍未实现；共享路径祖先替换 TOCTOU 也必须在不可信目录真实 Pilot 前独立关闭。
