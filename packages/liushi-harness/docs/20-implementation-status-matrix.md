...
| 08 Executor Adapter         | 已完成   | `partial`     | Codex Probe、Hook Binding、Pre/Post Adapter、CLI Wrapper、`hooks.json` 投影、G0 InstallPlan dry-run、G0 Apply、Installation Revision、Executor Compatibility Matrix Domain、Host Result v2 验证、Codex Compatibility Evidence Projector、内容寻址 Evidence/Matrix Store、重算查询 CLI、Fixture 测试和固定公开项目生产 CLI Cell Smoke                                  | 可追溯的真实 v2 Host Artifact、Contract Evidence、Rollback/Uninstall、Claude/CatPaw Adapter、Role Invocation                    |
...
- 当前 Codex 精确 Scope 只包含 Codex Adapter、Codex CLI Distribution、npm Tarball/Adapter Digest、Codex 版本、Interactive TUI、实测 OS/Architecture 和配置 Digest；不会从 Activation Plan 推断 Model 或 Permission。
- 投影固定输出 1 条 StaticProbe、4 条 SmokeTest 和 2 条 NegativeTest，绝不生成 ContractTest 或 ProductionE2e。由于缺少 Contract Evidence，且 Model/Permission Scope 尚未绑定，固定 Policy 编译的当前最高等级是 `experimental`，不能声明 `compatible` 或 `production`。
- `executor compatibility compile/query` 只接受三份原始 Host JSON 或精确 Matrix Digest；Application 固定选择受信 Policy，File Store 内容寻址保存脱敏 Artifact、Evidence、Policy 和 Matrix，Query 重新读取完整证据链并调用 Domain Compiler。
- 当前尚未实现可追溯真实 v2 Host Artifact、同 Scope Contract Evidence 或安装前支持查询；因此不能把投影器、Store/CLI 实现或历史旧版 Host Smoke 结果误述为已完成 Codex 可发布版本矩阵。
...
- Codex Host Result v2 的 `verify-result` 结果验证，以及对 Prepare v5、Activation Plan v2、Host Result v2 的脱敏 Compatibility Evidence 投影实现；该实现不等于仓库已有真实 v2 Host Artifact 或可发布版本矩阵。
...
- Executor Compatibility CLI：从三份原始 Codex Host JSON 编译并内容寻址持久化 Evidence、Policy 和 Matrix，按精确 Matrix Digest 跨进程重算查询。
...
- G0 Rollback/Uninstall、CLI recovery、Codex Host Hook 自动安装、可追溯的真实 v2 Host Artifact、Contract Evidence、其他工具/版本/平台验证、Claude-compatible 或 CatPaw Adapter。
