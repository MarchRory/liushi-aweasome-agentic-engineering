...
| 08 Executor Adapter         | 已完成   | `partial`     | Codex Probe、Hook Binding、Pre/Post Adapter、CLI Wrapper、`hooks.json` 投影、G0 InstallPlan dry-run、G0 Apply、Installation Revision、Executor Compatibility Matrix Domain、真实 Codex CLI 0.144.5 Host Result v2、Host 7 条 Evidence Projector、固定 v2 Contract Suite 5 条 ContractTest、双 Artifact Store、重算查询 CLI、确定性 Publication Bundle、create-only 原子输出 CLI、P3a G6 Domain、P3b/P4b3 包内 Sigstore Sign、P4a-P4b3 Manifest/Trust Profile、Manifest G6、可信 Release Approval Authority、Signed Manifest 与公开离线 Verify、Fixture 测试和固定公开项目生产 CLI Cell Smoke | 隔离 Release Host、真实企业审批源 Adapter、Attestation/Manifest Reader、Writer 与 CLI、Accepted Head、安装信任门、Rollback/Uninstall、Claude/CatPaw Adapter、Role Invocation |
...
因此，表格中 CodingTask Store/Command 的旧描述以本节为准；固定公开项目的预编排 Mutation CLI 正路径已有动态验证快照。历史 Codex Host Smoke 只保留旧版结果，不能作为 Host Result v2 Artifact；2026-07-16 已重新执行真实 Codex CLI `0.144.5` v2 Host，并通过 Compile/Query 形成本机 `compatible` Matrix。确定性 Publication Bundle、create-only 原子输出 CLI、P3a G6 Domain、P3b/P4b3 包内 Sign，以及 P4a-P4b3 Manifest/Trust Profile、Manifest G6、可信 Release Approval Authority、Signed Manifest 与公开离线 Verify 已实现。公共 npm API 与 `HarnessApplication` 已关闭签名面；隔离 Release Host、真实企业审批源 Adapter、Attestation/Manifest Reader、Writer 与 CLI、Accepted Head 和安装信任门仍未实现。Worktree 清理/重建、Import Graph、重试/Flaky、Waiver、多仓影响传播、真实 Agent 编码、企业安装和 Studio 仍未实现。
...
- G0 Rollback/Uninstall、CLI recovery、Codex Host Hook 自动安装、Attestation/Manifest Reader、Writer 与 CLI、Accepted Head、安装选择门、其他工具/版本/平台验证、Claude-compatible 或 CatPaw Adapter。
...
真实 Codex CLI `0.144.5`、Windows x64、Interactive TUI 的 12 条 Evidence 已全部 Passed，编译为本机 Compatible Matrix，并通过 Query 重算；Contract Failed Evidence 仍形成 Unsupported。确定性 Publication Bundle 已绑定双 Artifact、Evidence、Policy、Matrix、Tarball 与源码 Revision，并可由 create-only CLI 原子写出；P3a 已能生成绑定精确发布者身份、目标和 Human G6 Approval 的 in-toto Draft，P3b/P4b3 的包内签名链已能在可信 Authority 返回权威记录后生成真实 Sigstore DSSE Bundle 与 Signed Artifact，并以显式 Trusted Root 完成禁网离线验证；P4a-P4b3 已交付确定性 Manifest、Consumer Trust Profile、独立 Manifest Human G6、可信 Release Approval Authority、Signed Manifest Artifact 和完整双 DSSE 离线验证链。当前 npm 根和 `HarnessApplication` 不暴露 Signer、Sign UseCase 或 Authority 工厂；隔离 Release Host、真实企业审批源 Adapter、Artifact Reader/Writer、Accepted Head、安装选择门、ProductionE2E、`modelId` 或 `permissionMode` 尚未实现，因此绝不声明 Production。两类 Smoke 都不把自动化 Approval 计作日常业务 Human 决策，HTT 未测量，也不证明 Agent 自主编码或失败修复循环。下一步是 Attestation/Manifest Reader、Writer 与隔离 Release CLI，再补 Accepted Head 和安装选择门；之后进入 Claude-compatible/CatPaw Adapter。Rollback/Uninstall、CLI recovery、Worktree 清理/重建、失败分类/受限重试和多仓编排仍属后续能力。
