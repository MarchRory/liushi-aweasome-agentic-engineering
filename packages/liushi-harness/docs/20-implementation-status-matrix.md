...
| 08 Executor Adapter         | 已完成   | `partial`     | Codex Probe、Hook Binding、Pre/Post Adapter、CLI Wrapper、`hooks.json` 投影、G0 InstallPlan dry-run、G0 Apply、Installation Revision、Executor Compatibility Matrix Domain、真实 Codex CLI 0.144.5 Host Result v2、Host 7 条 Evidence Projector、固定 v2 Contract Suite 5 条 ContractTest、双 Artifact Store、重算查询 CLI、确定性 Publication Bundle、create-only 原子输出 CLI、P3a G6 Domain、P3b Sigstore Sign/Offline Verify、P4a Manifest/Trust Profile、Manifest G6 Draft Domain、Fixture 测试和固定公开项目生产 CLI Cell Smoke | Attestation CLI、签名 Artifact Writer、Signed Manifest Artifact/Verify、Accepted Head、安装信任门、Rollback/Uninstall、Claude/CatPaw Adapter、Role Invocation |
...
因此，表格中 CodingTask Store/Command 的旧描述以本节为准；固定公开项目的预编排 Mutation CLI 正路径已有动态验证快照。历史 Codex Host Smoke 只保留旧版结果，不能作为 Host Result v2 Artifact；2026-07-16 已重新执行真实 Codex CLI `0.144.5` v2 Host，并通过 Compile/Query 形成本机 `compatible` Matrix。确定性 Publication Bundle、create-only 原子输出 CLI、P3a G6 Domain、P3b Sigstore Sign/Offline Verify、P4a Manifest/Trust Profile 与 Manifest 独立 G6 Draft Domain 已实现，但 Attestation CLI、签名 Artifact Writer、Signed Manifest Artifact/Verify、Accepted Head 和安装信任门仍未实现。Worktree 清理/重建、Import Graph、重试/Flaky、Waiver、多仓影响传播、真实 Agent 编码、企业安装和 Studio 仍未实现。
...
- Executor Compatibility Attestation Library：重建真实 Human G6 Draft 后使用官方 Sigstore 生成 DSSE Bundle 与内容寻址 Artifact，并以调用方显式 Trusted Root 在禁网状态验证证书链、CT/TLog、可验证时间证据及精确发布者身份。
...
- G0 Rollback/Uninstall、CLI recovery、Codex Host Hook 自动安装、Attestation CLI/Writer、Signed Manifest Artifact/DSSE、Accepted Head、安装选择门、其他工具/版本/平台验证、Claude-compatible 或 CatPaw Adapter。
...
真实 Codex CLI `0.144.5`、Windows x64、Interactive TUI 的 12 条 Evidence 已全部 Passed，编译为本机 Compatible Matrix，并通过 Query 重算；Contract Failed Evidence 仍形成 Unsupported。确定性 Publication Bundle 已绑定双 Artifact、Evidence、Policy、Matrix、Tarball 与源码 Revision，并可由 create-only CLI 原子写出；P3a 已能生成绑定精确发布者身份、目标和真实 Human G6 Approval 的 in-toto Draft，P3b 已能生成真实 Sigstore DSSE Bundle 与 Signed Attestation Artifact，并以显式 Trusted Root 完成禁网离线验证；P4a 已交付确定性 Manifest 与 Consumer Trust Profile Domain，P4b1 已收敛通用 in-toto Sigstore 边界，P4b2 已交付独立 Manifest Human G6 Draft 与 in-toto Statement Domain。当前没有 Signed Manifest Artifact、显式 Trust Profile 离线验签、安装选择门、ProductionE2E、`modelId` 或 `permissionMode`，绝不声明 Production。两类 Smoke 都不把自动化 Approval 计作日常业务 Human 决策，HTT 未测量，也不证明 Agent 自主编码或失败修复循环。下一步是 Signed Manifest Artifact 与离线验证链，再补 Attestation/Manifest CLI/Writer 和安装选择门；之后进入 Claude-compatible/CatPaw Adapter。Rollback/Uninstall、CLI recovery、Worktree 清理/重建、失败分类/受限重试和多仓编排仍属后续能力。
