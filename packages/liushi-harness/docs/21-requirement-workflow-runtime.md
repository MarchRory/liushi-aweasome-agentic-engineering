...
当前实现仍属于 Harness 前置机制：持久化 Command Gateway/Receipt、Revision/Context、Golden Replay、Timeline、Action Journal、JournaledActionRunner、Trace、Canonical Hook Core、RequirementWorkflow Domain/Store/Command，以及 CodingTask Domain/Store/Command、权威授权、Managed Worktree Provision/Inspector、未知 Provision Human 对账、未闭合 Provision 下游 Guard、Repository Lock、受控文件变更、可信 Repository Root、Git Checkpoint、ImplementationSubmitted、版本化 Verification Command、G8 Profile-backed 单仓影响面、Local Command Runner 和 EvidenceBundle Store 已落地。现有 CLI 写命令尚未全部迁移到 Gateway，平台安装、实时 Span、OTel Exporter、Cell Runtime、Worktree 清理/重建、Import Graph、多仓影响传播、失败重试和 Studio 均未实现。
...
| S3 CodingTask 与 Verification | CodingTask Domain Core、独立 Schema、File Store/Replay、Command Service、权威 ExecutionAuthorization、Attempt/Verification 状态机、Human Resolution、Managed Worktree、未知 Provision Human 对账与下游 Guard、Repository Lock、JournaledActionRunner、受控文件变更、可信 Root、Git Checkpoint、ImplementationSubmitted、VerificationPlan/Command/EvidenceBundle、G8 Profile-backed 单仓影响面、fail-closed Mock Executor | Worktree 清理/重建、Import Graph、多仓影响传播、受限重试策略；崩溃可恢复、环境失败不进入 Bug Fix、需求歧义返回 Human                                               |
...
当前仍缺少 Worktree 清理/重建、重试/Flaky、Waiver、Child Workflow 和 Cell Runtime；Workflow Studio 仍是同级独立包。Worktree Provision 与 Recovery 不执行删除、Reset、Checkout、Merge、自动修复或自动重试；Local Command Runner 只负责显式 Plan 的受限执行，不等同于完整 Verification Cell；当前也不提供 Agent 自主代码生成、PR、推送、合并、发布或部署。
