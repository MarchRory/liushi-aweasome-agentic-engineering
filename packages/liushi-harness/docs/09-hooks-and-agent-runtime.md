...
Codex Agent Runtime Isolation 已迁入正式 Infrastructure：Runtime Plan、环境白名单、auth.json 元数据快照与隔离副本、Windows/POSIX 权限策略、外部 Skill/Config/MCP 阻断和精确清理由唯一生产实现负责，固定公开项目 Pilot 只保留构建产物桥。Codex App Server Runner 也已迁入正式 Agent Host Infrastructure：严格协议状态机、File Change Human Gate、路径 Allowlist、Canonical Evidence、输出限制、跨平台进程树终止与 `outcome_unknown` 由唯一生产实现负责，旧 Runner 同样只保留构建产物桥。Codex Agent Host Session Flags 已完成正式化，负责确定性 `-c key=TOML`、Runtime key 路径保护、固定 Provider/reasoning、受限 Hook/Trust 和旧 exec 兼容。零模型 Preflight 现已通过两个隔离的真实 Codex 进程生成绑定精确可执行文件身份的 v1 Evidence，并由 Host Approval Packet v3 消费；Application Host 生命周期仍待后续实现。以上切片仍不等于完整 Agent Host。
...
- `pending`、缺失 PostAction、`WaitingHuman`、`RetryPermitted` 或 `outcome_unknown` 都会阻止 `beginClosing`。S3 Process Manager 会在 Repository Lock 内驱动 Action/Trace Coverage、Snapshot 和 Checkpoint，并由独立 CLI 停在 `CheckpointBound`；随后 Completion CLI 通过独立 Delivery Command 接入 CodingTask，并继续执行权威 Verification、Evidence 与 PRReady 装配。PR 创建、推送和合并仍由 Human 负责。
