...
发布前必须通过 Format、Lint、Markdown、Typecheck、Test、Architecture Test、Build、Package Content 和 Changeset Status。npm 发布由 CI 使用 Provenance 完成，个人本地环境默认不直接发布正式版本。

CI 在 Linux 和 Windows 上执行相同 Quality Gate。GitHub Actions 使用完整 Commit SHA 固定第三方 Action，并由 Dependabot 提交升级 PR；Release Workflow 只在 `main` 上创建 Version PR 或发布 Changesets 已准备的版本。
