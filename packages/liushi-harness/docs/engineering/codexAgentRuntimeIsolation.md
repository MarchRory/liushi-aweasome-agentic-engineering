...
Runtime Isolation 完成后，下一切片迁移 App Server Runner，并拆分现有大协议文件。
随后接入 Session Flags 与零模型 Preflight，最后由 Application Host 编排
`preview -> approve -> run -> status`。固定公开项目 Pilot 将持续作为回归 Fixture，
企业单仓 Pilot 才负责产生真实生产与量化证据。
