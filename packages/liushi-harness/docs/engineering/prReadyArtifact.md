...
真实 Git E2E 已覆盖 G1/G4 Human Approval、默认 Task-backed 授权、文件变更、单一 Checkpoint、Checkpoint 后权威解析 Verification Target Revision、Local Command Verification、Artifact 装配和跨 Application 实例零重复副作用；Windows 本地 Tarball 干净安装 Smoke 已通过。公开或受信任项目的真实 Hook/受控闭环 Smoke、多仓依赖传播和 Human Touch Time 采集尚未完成。
