  ...
  it("Human help 输出全部真实命令，且只写入 stdout", async () => {
    await withStore(async (storeRoot) => {
      ...
      expect(singleOutput(output.stdout).split("\n").filter(Boolean)).toEqual([
        ...
        expect.stringContaining("init --apply"),
        ...
        expect.stringContaining("requirement confirm"),
        expect.stringContaining("plan-risk analyze"),
        expect.stringContaining("plan-risk confirm"),
        ...
        expect.stringContaining("coding-task session metrics enroll"),
        expect.stringContaining("coding-task session metrics settle"),
        expect.stringContaining("coding-task session metrics report"),
        expect.stringContaining("coding-task session closeout assess"),
        expect.stringContaining("coding-task session closeout recover"),
        expect.stringContaining("coding-task session closeout effective"),
        ...
        expect.stringContaining("executor compatibility bundle create"),
      ]);
    });
  });
});
