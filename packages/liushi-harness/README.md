...
- Codex App Server Runner 已迁入正式 Agent Host Infrastructure，提供严格 JSONL/JSON-RPC 状态机、Human-gated File Change、路径 Allowlist、Canonical Approval Evidence、输出限制、跨平台进程树终止和 `outcome_unknown`；旧 Pilot 的 Runner 目录只保留唯一构建产物桥。该切片仍不等于完整 Agent Host，Session Flags、零模型 Preflight 和 Application Host 生命周期尚未实现。
