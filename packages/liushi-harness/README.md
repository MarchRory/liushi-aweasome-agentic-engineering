...
- 真实 Codex App Server Pilot 已在同一固定公开项目、单文件 Write Set 和单次模型启动下完成 Agent 变更、Session Closeout、唯一 Checkpoint、Delivery、项目测试、类型检查与 PR-ready 装配；该技术 Pilot 不等于企业项目生产声明或量化提效结论。
...
- Codex App Server Runner 已迁入正式 Agent Host Infrastructure，提供严格 JSONL/JSON-RPC 状态机、Human-gated File Change、路径 Allowlist、Canonical Approval Evidence、输出限制、跨平台进程树终止和 `outcome_unknown`；旧 Pilot 的 Runner 目录只保留唯一构建产物桥。该切片仍不等于完整 Agent Host，零模型 Preflight 和 Application Host 生命周期尚未实现。
- Codex Agent Host Session Flags 已迁入正式 Infrastructure，提供确定性 `-c key=TOML` 组装、受限 Runtime key 保护、固定生产 Provider/reasoning、受限 Hook/临时 Trust 编码和旧 exec 兼容；旧 Pilot 只保留唯一 dist bridge。该切片只完成 Session Flags 正式化，Preflight 与 Application Host 仍未完成。详见 [Codex Agent Host Session Flags](./docs/engineering/codexSessionFlags.md)。
...
现有 CLI 写命令向 Application Command Gateway 的完整迁移、企业项目 Pilot 与量化、G0 Rollback/Uninstall、隔离 Release Host、Attestation/Manifest CLI 与 Artifact I/O 接线、Accepted Head 与安装信任门、Claude-compatible/CatPaw 平台适配、实时 Span 生命周期与 OTel Exporter、完整 RequirementWorkflow Cell Runtime、Worktree 清理/重建、多仓写入编排、Import Graph 与多仓 Verification 影响传播、重试/Flaky、Waiver、完整 Agent Host Runtime（当前已正式化 Codex Runtime Isolation 与 App Server Runner）、其他 Canonical 生命周期事件、Validator Execution、Instruction Projection、Memory、Skills、Connectors 和 Profile 持久化 Registry 仍属于后续实现范围。当前包仍不声明完整生产闭环；公开项目 Pilot 只证明其精确单仓边界。Session Completion 独立 CLI 已交付，下一主线进入脱敏企业需求 Pilot；Release Host、Studio、Skills/Memory/Connectors、Claude/CatPaw 扩张继续冻结。当前 P3b/P4b3 的签名实现、P4c1 Artifact Reader/Writer 和 P4c2a 企业 HTTPS Authority Adapter 都只作为包内待装配能力存在；npm 公共 API 与 `createHarnessApplication` 不提供签名入口，离线 Verify 仍是公开能力。Profile Promotion 和 InstallPlan dry-run 都不写入业务配置，G0 Apply 也不代表代码已经通过合规验证。
