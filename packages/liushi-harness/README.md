...
- CodingTask 已提供单仓 Aggregate、独立 Schema、File Store/Replay、Versioned Command Gateway/Service、权威 ExecutionAuthorization、G2 历史逻辑确认绑定、Attempt 串行状态机、Verification 结果接纳和 Human Resolution；Worktree Inspector 已能只读检查真实 Worktree、Base Revision、分支、Git Diff 和 canonical Write Set；Repository Lock、VerificationPlan、Verification Port、EvidenceBundle 装配和 fail-closed Mock Executor 已落地，仍不包含 Worktree 生命周期管理或真实命令执行器。

现有 CLI 写命令向 Application Command Gateway 的完整迁移、Codex 真实受信任项目安装与 Smoke、Claude-compatible/CatPaw 平台适配、实时 Span 生命周期与 OTel Exporter、Cell Runtime、Worktree 创建/清理与受控写入、多仓写入编排、真实 Verification Command Runner、影响面选择、重试/Flaky、Waiver、Agent Runtime、其他 Canonical 生命周期事件、Validator Execution、Instruction Projection、Memory、Skills、Connectors 和 Profile 持久化 Registry 仍属于后续实现范围。当前 Profile Promotion 只生成可审计 Bundle，不写入业务仓库，也不代表代码已经通过合规验证。
...
本轮仍未提供 Worktree 创建/管理、受控写入、跨仓 Saga、真实 Verification Command Runner、影响面选择、重试/Flaky、Waiver、真实 Executor、完整 Workflow Runtime、Skills/Memory/Connectors 或 Studio；Repository Lock 只提供运行时互斥，不创建 Worktree、不修改 Git、不授予写权限；当前 Mock 只用于契约和测试，未配置 Check 必须返回 `Blocked`，不能因为它存在而视为生产验证已完成。
