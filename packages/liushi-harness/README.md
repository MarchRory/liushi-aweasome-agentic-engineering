...
- Rollback 和 Uninstall 当前未实现；Closeout Recovery CLI 已接入，但故障注入与真实 Git 恢复 E2E 尚未完成。`actor-id` 只是审计身份声明，不是认证机制，企业使用须由外部受信任包装器或身份系统注入。
...
- Closeout Recovery 已具备只读 `assess`、完整 Human Command `recover` 与只读 `effective` 三个 CLI 入口。恢复命令复用唯一 Application Command Gateway，在 Repository Lock 内 fresh reassessment，并通过独立 Recovery State 的 create-only/CAS 保证仅 Intent winner 至多执行一次 Checkpoint；CLI 不接受直接 Resolution 参数，也不允许直接写 Recovery State 或 Checkpoint。故障注入和真实 Git 恢复 E2E 仍未完成，因此不声明恢复生产闭环。
