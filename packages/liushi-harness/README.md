...
本轮仍未提供 Worktree 清理/重建、受控代码写入、跨仓 Saga、真实 Verification Command Runner、影响面选择、重试/Flaky、Waiver、完整 Workflow Runtime、Skills/Memory/Connectors 或 Studio；Worktree Provision 仅创建 CodingTask 已声明的 managed 绑定，不删除、Reset、Checkout 或自动修复既有历史；当前 Verification Mock 只用于契约和测试，未配置 Check 必须返回 `Blocked`。
